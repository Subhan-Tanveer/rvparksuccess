...
/* -- Website Design: Basic vs Premium comparison -- */
const websiteCompare = document.getElementById('websiteCompare');
if (websiteCompare) {
  websiteCompare.innerHTML = WEBSITE_TIERS.map((s) => `
    <div class="tilt-card website-card${s.featured ? ' is-premium' : ''}">
      ${s.featured ? '<span class="badge-pop">Premium</span>' : ''}
      <h3>${s.name.replace('Website Design — ', '')}</h3>
      ${priceRowHtml(s)}
      <p class="lede" style="font-size: 0.875rem;">${s.desc}</p>
      <ul class="feature-list">${s.features.map((f) => `<li>${CHECK}${f}</li>`).join('')}</ul>
      <button class="btn ${s.featured ? 'btn-primary' : 'btn-ghost'} btn-get-started magnetic" data-key="${s.key}"><span>Get Started</span></button>
    </div>`).join('');
}
...
if (servicesGrid) {
  servicesGrid.innerHTML = SERVICES.map((s) => `
    <div class="tilt-card service-card">
      ...
      ${priceRowHtml(s)}
      <div class="pause-note">${PAUSE_ICON} Pause anytime — seasonal flexibility built in</div>
...
/* -- Payment modal: works for any fixed-price service (website tiers + individual services) -- */
const ALL_FIXED = [...WEBSITE_TIERS, ...SERVICES];
